    ...
    <div className="min-h-screen bg-[#F8F9FA]">
        ...
        <div className="max-w-lg mx-auto">
          ...
          <div className="flex bg-gray-100 p-1 rounded-xl gap-1">
            {(['scans', 'recipes'] as Tab[]).map(t => (
              <button key={t} onClick={() => setTab(t)} className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-all ${tab === t ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500'}`}>
      ...
      <main className="max-w-lg mx-auto px-4 py-5">
        {loading ? (
          ...
          scans.length === 0 ? <EmptyHistory label="scans" /> : (
            <div className="space-y-3">
              {scans.map(scan => (
                <div key={scan.id} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                  <div className="flex items-start justify-between gap-3">
                    <div className="flex-1 min-w-0">
                      <p className="text-xs text-gray-400 mb-2">{new Date(scan.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long', year: 'numeric' })}</p>
                      <div className="flex flex-wrap gap-1.5">
                        {scan.ingredients.slice(0, 6).map((ing, i) => <span key={i} className="bg-gray-100 text-gray-600 text-xs font-medium px-2 py-0.5 rounded-lg">{ing.name}</span>)}
                        {scan.ingredients.length > 6 && <span className="text-xs text-gray-400">+{scan.ingredients.length - 6}</span>}
                      </div>
                    </div>
                    <button onClick={() => replayScan(scan)} className="shrink-0 bg-green-50 hover:bg-green-100 text-green-600 text-xs font-semibold px-3 py-1.5 rounded-xl transition-colors">Relancer</button>
                  </div>
                </div>
              ))}
            </div>
          ...
          recipes.length === 0 ? <EmptyHistory label="recettes" /> : (
            <div className="space-y-3">
              {recipes.map(recipe => (
                <div key={recipe.id} className="bg-white rounded-2xl p-4 border border-gray-100 shadow-sm">
                  <p className="text-xs text-gray-400 mb-1">{new Date(recipe.created_at).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}</p>
...
function EmptyHistory({ label }: { label: string }) {
  return (
    <div className="text-center py-12">
      ...
      <Link href="/scan" className="inline-block mt-4 bg-green-500 text-white text-sm font-bold px-6 py-3 rounded-xl transition-colors hover:bg-green-400">Scanner maintenant</Link>
    </div>
  )
}
