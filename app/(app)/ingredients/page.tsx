...
export default function IngredientsPage() {
  ...
  function remove(id: string) { setIngredients(p => p.filter(i => i.id !== id)) }
  function startEdit(item: Ingredient) { setEditingId(item.id); setEditValue(item.name) }
  function saveEdit(id: string) {
    if (editValue.trim()) setIngredients(p => p.map(i => i.id === id ? { ...i, name: editValue.trim() } : i))
    setEditingId(null)
  }
  ...
  async function findRecipes() {
    ...
    setError(null)
    try {
      const scanId = sessionStorage.getItem('frigochef_scan_id')
      const res = await fetch('/api/recipes', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ingredients: ingredients.map(i => i.name), scanId }),
      })
  ...
  return (
    <div className="min-h-screen bg-[#F8F9FA] flex flex-col">
      ...
      <main className="flex-1 px-6 py-8 pb-36">
        <div className="max-w-lg mx-auto">
          <div className="mb-6">
            ...
            <p className="text-gray-500 text-sm">{ingredients.length} détecté{ingredients.length !== 1 ? 's' : ''}. Tapez pour modifier, ✕ pour supprimer.</p>
          </div>

          <div className="flex flex-wrap gap-2 mb-6 animate-fade-in">
            {ingredients.map(item => (
              <div key={item.id} className="flex items-center bg-white border border-gray-200 rounded-xl overflow-hidden shadow-sm">
                {editingId === item.id ? (
                  <input
                    autoFocus
                    value={editValue}
                  ...
                  <button onClick={() => startEdit(item)} className="flex items-center gap-1.5 px-3 py-2 text-sm font-medium text-gray-800 hover:bg-gray-50 transition-colors">
                    {item.name}
                    {item.confidence < 1 && <span className="text-[10px] text-gray-300">{Math.round(item.confidence * 100)}%</span>}
                  </button>
                )}
                <button onClick={() => remove(item.id)} className="pr-2.5 py-2 text-gray-300 hover:text-red-400 transition-colors text-xs">✕</button>
              </div>
            ))}
            {ingredients.length === 0 && <p className="text-gray-400 text-sm py-4">Aucun ingrédient — ajoutez-en ci-dessous.</p>}
          </div>

          <div className="border-t border-gray-100 mb-6" />
          <p className="text-xs font-semibold text-gray-400 uppercase tracking-wider mb-3">Ajouter un ingrédient</p>
          <div className="flex gap-2">
            ...
            <button
              ...
              className="bg-gray-900 hover:bg-gray-700 disabled:opacity-30 text-white px-4 py-3 rounded-xl transition-colors"
            >
      ...
      <div className="fixed bottom-0 left-0 right-0 px-6 pb-24 pt-4 bg-gradient-to-t from-[#F8F9FA] via-[#F8F9FA]/95 to-transparent">
        <div className="max-w-lg mx-auto">
          <button
            onClick={findRecipes}
            disabled={loading || ingredients.length === 0}
            className="w-full bg-green-500 hover:bg-green-400 active:scale-[0.98] disabled:opacity-50 text-white font-bold py-5 rounded-2xl text-base shadow-lg shadow-green-500/25 transition-all"
          >
            {loading ? 'Génération en cours…' : 'Trouver des recettes →'}
          </button>
          <p className="text-center text-xs text-gray-400 mt-2">{ingredients.length} ingrédient{ingredients.length !== 1 ? 's' : ''} · 5 recettes générées par l&apos;IA</p>
        </div>
      </div>
      ...
      {loading && (
        <div className="fixed inset-0 bg-white/95 backdrop-blur-sm flex flex-col items-center justify-center z-50 animate-fade-in">
          ...
          <p className="text-gray-900 font-bold text-lg">Création de vos recettes…</p>
          <p className="text-gray-400 text-sm mt-1">Le chef IA travaille pour vous</p>
        </div>
      )}
    </div>
  )
