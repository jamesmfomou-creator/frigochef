...
export default function PlanningPage() {
  ...
  async function loadPlan() {
    ...
    setLoading(true)
    const { data } = await supabase.from('meal_plans').select('*').order('created_at', { ascending: false }).limit(1).single<MealPlan>()
    setPlan(data)
    setLoading(false)
  }
  ...
  async function generateWeek() {
    ...
    try {
      ...
      const res = await fetch('/api/planning', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ pantryIngredients }) })
      ...
      if (!user) return
      const { data: saved } = await supabase.from('meal_plans').upsert({ user_id: user.id, week_start: getMonday(), days: data.days }, { onConflict: 'user_id,week_start' }).select().single<MealPlan>()
      setPlan(saved)
    ...
    <div className="min-h-screen bg-[#F8F9FA]">
      <header className="bg-white border-b border-gray-100 px-6 pt-8 pb-5">
        <div className="max-w-lg mx-auto">
          ...
          <p className="text-gray-400 text-sm">{plan ? `Semaine du ${new Date(plan.week_start).toLocaleDateString('fr-FR', { day: 'numeric', month: 'long' })}` : 'Générez votre semaine de repas'}</p>
        </div>
      </header>
      <main className="max-w-lg mx-auto px-4 py-5">
        <button onClick={generateWeek} disabled={generating || loading} className="w-full bg-green-500 hover:bg-green-400 active:scale-[0.98] disabled:opacity-60 text-white font-bold py-4 rounded-2xl transition-all mb-5 shadow-lg shadow-green-500/20 flex items-center justify-center gap-2">
          {generating ? <><span className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />Génération en cours…</> : <>{plan ? '🔄 Regénérer ma semaine' : '✨ Générer ma semaine'}</>}
        </button>
        {loading ? (
          ...
          <div className="space-y-3">
            {DAYS_FR.map(day => {
              ...
              return (
                <div key={day} onClick={() => meal && setSelectedMeal({ day, meal })} className={`bg-white rounded-2xl p-4 border shadow-sm transition-all ${meal ? 'cursor-pointer hover:shadow-md' : 'opacity-60'} ${isToday ? 'border-green-200 ring-2 ring-green-100' : 'border-gray-100'}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                      <div className={`text-xs font-black w-10 text-center py-1 rounded-lg ${isToday ? 'bg-green-500 text-white' : 'bg-gray-100 text-gray-500'}`}>{DAY_LABELS[day].slice(0, 3).toUpperCase()}</div>
                      <div>
                        {meal ? <><p className="font-bold text-gray-900 text-sm">{meal.title}</p><p className="text-xs text-gray-400">{meal.time} · {meal.calories}</p></> : <p className="text-sm text-gray-400">Aucun repas</p>}
                      </div>
                    </div>
      ...
      {selectedMeal && <MealModal meal={selectedMeal.meal} day={selectedMeal.day} onClose={() => setSelectedMeal(null)} />}
    </div>
  )
}
